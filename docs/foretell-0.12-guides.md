...
Since 0.12.3, a guide with supported boss headings and non-empty narrative text is accepted even when it contains no extracted named mechanics. Previously, both parsing and cache validation incorrectly required at least one named mechanic, causing `InvalidDataException` for Kugane Castle. Narrative advice remains attached to its own boss and is eligible for the existing context-summary worker/cache and session analysis export. The entry panel explicitly states the lack of automatic guide matching/alerts; the transparent overlay shows a short narrative-guide notice with advice available on boss hover. No paragraph or inline ability mention is promoted into an invented executable mechanic. Empty boss sections, non-boss pages and wrong duty variants remain rejected. Parser failures now retain a bounded diagnostic message, and the entry panel offers a retry rather than requiring an instance change.
