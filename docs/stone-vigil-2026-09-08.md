...
- Short cues have a separate complete/shared scope. Model instructions keep all conditional alternatives in responses and description, not in the central cue. Both list and centre use the same short-cue selection. The initial 0.13.11 legacy fallback used an avoidance-specific regular expression based on this report; it was not a general AI summarizer. That fallback is removed in 0.13.12. Model-generated scoped cues are displayed unchanged. Older guides retain an already-short complete instruction, or just the mechanic name when the complete instruction is too long. Reanalyzing an older guide generates scoped short cues; updating the plugin alone does not regenerate cached text. Full details remain available.
