...
Guide content and player instructions remain in English; application controls can follow the client language. Three pinned local profiles are available: **Qwen3.5 4B (default)**, Gemma 4 E2B and Granite 4.1 3B. Real runs prepared Praetorium (3 bosses, 21 mechanics) and Orbonne (4 bosses, 50 mechanics). Direction, naming and conditional-response errors remain; these counts are not a quality score. New semantic preparation requires a local model; accepted prepared caches can be shown without keeping it loaded. The hidden helper pauses for combat and can resume incomplete work. First setup and analysis can take minutes. Context and RAM limits do not cap VRAM or guarantee in-game performance. See [local models and validation status](docs/foretell-models-2026-09-06.md).
