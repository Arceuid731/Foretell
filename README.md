...
The radar can be unlocked and dragged, resized independently in pixels, zoomed in world yalms, or forced to a circle/square. In Auto mode Foretell copies nearby collision triangles already loaded by FFXIV, then rasterizes a layered heightfield and its player-reachable component on a managed worker. Walkable PCB faces reconstruct rooms, corridors, ramps and stairs; steep faces plus primitive box/cylinder/plane colliders provide walls and invisible barriers without hard-coded territory knowledge. Its world-aligned rolling window extends well beyond the visible radar and recentres early, so movement cannot expose a scan edge. Rebuilds are double buffered: a complete prior map stays visible until the complete replacement is atomically swapped, and the fallback never publishes scan fragments. Connected contours are simplified and rendered as continuous top-down wall/drop lines rather than isolated cell edges. A lightweight scene fingerprint plus combat and structural events promptly invalidate changing floors and barriers even while the player stands still. The same topology constrains safe-position searches, clips known-unreachable portions of 2D/3D attack outlines and projects world overlays onto the reconstructed floor; unknown terrain remains visible rather than causing a false-negative alert. A separately capped combat radial sweep overlays fresh pull barriers, and the earlier near-first raycast frontier now serves only as a compatibility fallback when collision mesh data is unavailable.
