...
The mechanic list is controller-friendly: no pages or navigation buttons, automatic columns, distinct name/action lines, role icons and optional background. Configure its typography, colors and spacing in **Display → Mechanic list**. **Follow the current phase** retains common mechanics and all active signals; until a phase is identified, all boss mechanics remain eligible. Phases are extracted by the model from source evidence and identified live through phase-specific observed mechanics, not elapsed time. Older cached guides remain usable without reanalysis but need an explicit **Reanalyze** to acquire the new phase metadata when their sources document phases.

Opening a guide manually prepares the same cache used at instance entry. Unchanged text/layout and transient source failures do not force inference again. Source updates are checked while the cached guide remains available; actual content changes or explicit reanalysis can prepare a replacement.

Conditions and alternative instructions stay intact. If an unusually dense list cannot fit even after adapting columns and scale, it scrolls automatically; active mechanics are brought into view without input. Widen or enlarge the unlocked overlay to keep more rows visible simultaneously.
