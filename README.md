...
Version 0.11 adds the first hybrid guide path: Foretell automatically retrieves the current instance's Console Games Wiki page, prepares a local boss/phase/mechanic document, and highlights uniquely matched live boss casts. There is no fixed list of supported instance IDs or user-supplied URL requirement. Page structure, unresolved identities and tactical variants still limit coverage; a downloaded guide is not a validated encounter solver.

Open `/foretell` → **Guides** to view acquisition/cache state, refresh a guide or search the client duty catalogue and prepare another instance outside combat. Enable **Hybrid** or **Foretell** mode for the sidebar and central hints. The current instance loads automatically, including on the first visit. Documents remain usable offline from `foretell-guides`; a failed refresh keeps the previous document. Source revision and retrieval date remain visible.

Official names identified from contextual game signals use the client language, as do the small set of simple prepared instructions (EN/FR/DE/JA). Names without a contextual ID binding stay marked **[EN]**; complex prose remains accessible as English source with **response not prepared**. Full automatic tactical extraction/translation is not implemented in this release. Conditions, nested lists and continuation paragraphs are retained. A cast match alone does not identify every variant, status response or safe zone. Radar/world geometry still comes from the existing game-evidence path; the wiki does not manufacture a shape or raise learning confidence. See [the hybrid guide review](docs/review-hybrid-guides-2026-09-06.md).
