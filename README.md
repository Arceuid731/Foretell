...
Version 0.12 connects the automatic Console Games Wiki guide to a current/upcoming-boss checklist, central cast alerts and the existing radar/world decision frame. An entry panel shows the source, quick topic summary, download/preparation progress and measured timing estimates. There is no fixed list of instance IDs or user-supplied URL requirement. A downloaded guide is still not a validated encounter solver.

Open `/foretell` → **Instance** for the current guide/boss; **Sources & guides** for catalogue/downloads; **Local AI** for model activity and resource settings; **Display** for the mechanic list, entry panel, central alerts, radar and 3D; **Advanced** for observed memory, recordings and Analysis ZIP. Enable **Hybrid** or **Foretell** mode for live presentation. The current instance loads automatically; cached documents work offline and failed refreshes preserve them. Only the identified current boss appears during combat; confirmed death advances the upcoming boss, wipes reset per-pull checkmarks, and simultaneous/ambiguous boss identities abstain. The inspector can still browse the entire document.

Version 0.12.1 replaces the document-style combat window with a transparent, borderless checklist: **mechanic — short response**, one line per mechanic, highlighted with a countdown while active. Descriptions, conditions and preparation details move to hover tooltips; hold Shift over a mechanic for the full English source. Locked height fits the rows automatically, with bounded pages rather than scrolling for unusually long lists; a live mechanic brings its page into view. Unlock temporarily restores drag/resize controls. The same response appears in the central alert, with its own text-scale setting. Unknown responses remain **CHECK RESPONSE**, not guessed orders. The entry panel now waits for manual dismissal, hides temporarily in combat, and can be reopened by clicking the boss heading.

Official names identified by contextual Action/Status/BNpcName IDs use the client language; unresolved names retain **[EN]**. The quick guide is usable without a model. Optional/default-enabled local translated summaries install a pinned Qwen3-1.7B model (1.83 GB) and llama.cpp runtime (18–35 MB) automatically, in a hidden auxiliary process with CPU/RAM limits. Inference stops during combat; prepared summaries remain available offline. Initial model setup can take minutes and is reported separately, never disguised as a fast wiki download. Summaries are documentary, may be imperfect, and never choose live targets or geometry. Source conditions remain accessible.

Cast/status associations synchronize checklist highlights, countdown alerts and guide labels on existing 2D/3D geometry, without raising learned confidence. Without established geometry, only an explicitly non-spatial signal annotation is drawn. Exact boss ownership can associate a helper; unrelated helpers and unknown marker/tether variants are not guessed. Conditional directions without observed prerequisites remain **unresolved**. See the [0.12 implementation and validation notes](docs/foretell-0.12-guides.md) and the historical [0.11 review](docs/review-hybrid-guides-2026-09-06.md).
...
Since 0.12.2, **Analysis ZIP** also includes `guides/index.json`, revision-bound extracted source documents, and session-time adapted snapshots: localized names with contextual IDs, conditions, local summaries, short responses, relevant guide settings and sampled live matches. These survive leaving the duty and are exported from the selected session, never replaced by the current cache. Older captures explicitly report guide evidence unavailable. See the [Arboretum 0.12 capture review](docs/review-arboretum-2026-09-06.md).
...
Version 0.12.4 reorganizes the cockpit around guides instead of first-run blind learning. Real auxiliary-process activity is distinct from cache readiness, visible in the header, entry panel and Local AI. Context is configurable (4K–32K, default 16K), with full-template token counting and explicit overflow rejection instead of silently skipping excerpts above 7000 characters. Committed RAM is configurable (4–12 GiB, default 6); no VRAM cap or in-game performance guarantee is claimed. Analysis snapshots retain these session-time settings and runtime state. The pinned Qwen3-1.7B model remains unchanged: whole-page AI extraction, additional guide providers and a validated model selector are **not implemented yet**. See the [compact-model comparison and delivered scope](docs/foretell-models-2026-09-06.md).
...
The adaptive learner learns from the following encounter-agnostic evidence. External guides are separately attributed documentary knowledge, explicitly authorized for the hybrid presentation; they do not become learned observations, prediction successes or confidence scores.
...
Foretell must not import BossModule/Splatoon mechanics, state machines, boss components, encounter layouts/presets or disguise authored answers as automatic learning. The separate wiki adapter may retrieve documented encounter knowledge with provenance. CI preserves the legacy-module separation and telemetry contract; the in-game coverage audit reports sensors that are truncated, unavailable or not explicitly classified.
