    ...
    private void DrawTelemetryStatus()
    {
        ...
        if (ImGui.BeginTable("ForetellTelemetryStatus", 3, ImGuiTableFlags.RowBg | ImGuiTableFlags.BordersInnerV | ImGuiTableFlags.SizingStretchProp))
        {
            ...
            DrawTelemetryRow("Native collision topology", !topologyHealthy ? "SAFE COOLDOWN" : CurrentArenaBoundary != null || _topologyAnalysis != null ? "ACTIVE" : "SCANNING", _topologyMeshPrimary
                ? $"PCB heightfield; {_topologyMeshTriangles:N0} triangles / {_topologyMeshColliders:N0} colliders; {_topologyMeshFloorTriangles:N0} floor + {_topologyMeshWallTriangles:N0} wall; capture {_topologyMeshCaptureMilliseconds:F2} ms (peak {_topologyMeshPeakCaptureMilliseconds:F2}); worker raster {_topologyMeshRasterMilliseconds:F2} ms (peak {_topologyMeshPeakRasterMilliseconds:F2}); {_topologyMeshFallbacks:N0} fallbacks / {_topologyFailures:N0} rejects"
