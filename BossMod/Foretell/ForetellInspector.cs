...
public sealed partial class ForetellEngine
{
    ...
    private void DrawTelemetryStatus()
    {
        ...
        if (ImGui.BeginTable("ForetellTelemetryStatus", 3, ImGuiTableFlags.RowBg | ImGuiTableFlags.BordersInnerV | ImGuiTableFlags.SizingStretchProp))
        {
            ...
            DrawTelemetryRow("Native collision topology", !topologyHealthy ? "SAFE COOLDOWN" : CurrentArenaBoundary != null || _topologyAnalysis != null ? "ACTIVE" : "SCANNING", _topologyMeshPrimary
                ? $"Rolling PCB heightfield; {_topologyMeshTriangles:N0} triangles / {_topologyMeshColliders:N0} colliders; {_topologyMeshFloorTriangles:N0} floor + {_topologyMeshWallTriangles:N0} wall; capture {_topologyMeshCaptureMilliseconds:F2} ms / {_topologyMeshCaptureBudget:F1} ms budget ({_topologyMeshCaptureTimeouts:N0} fast retries); worker raster {_topologyMeshRasterMilliseconds:F2} ms; {_topologyAtomicSwaps:N0} atomic swaps / {_topologyRetainedRebuilds:N0} retained fronts / {_topologyWindowRecenters:N0} recenters; scene {_topologySceneFingerprint:X16} / {_topologySceneFingerprintChanges:N0} changes / {_topologySceneColliders:N0} colliders; refresh {_topologyLastRefreshLatencyMilliseconds:F0} ms (peak {_topologyPeakRefreshLatencyMilliseconds:F0}); {_topologyMeshFallbacks:N0} fallbacks / {_topologyFailures:N0} rejects"
                : $"Raycast fallback; {_topologyFrontier.Reachable:N0} reached / {_topologyFrontier.Sampled:N0} sampled / {_topologyFrontier.Pending:N0} pending; first surface {_topologyFirstSurfaceMilliseconds:F0} ms; sweep {_topologyLastSweepMilliseconds:F0} ms (peak {_topologyPeakSweepMilliseconds:F0}); {_topologyFloorSamples:N0} floor + {_topologyEdgeSamples:N0} barrier rays; frame {_lastTopologyMilliseconds:F2} ms (peak {_peakTopologyMilliseconds:F2}); {_topologyOverruns:N0} overruns / {_topologyFailures:N0} rejects");
