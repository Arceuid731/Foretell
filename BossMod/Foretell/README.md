...
Auto radar mode builds a bounded walkable surface directly from the collision scene already loaded by FFXIV, with no vnavmesh runtime dependency and no authored territory data. Foretell copies only nearby PCB and primitive-collider triangles while it is on the framework thread. An immutable managed snapshot then crosses to a worker, which projects walkable triangles into a layered heightfield, turns steep faces into wall segments and floods only the layer reachable from the player. This is the same broad collision-mesh → raster → reachable-region architecture used by navmesh builders, reduced to the local 2D radar problem without Detour pathfinding or movement control.

The rolling window is aligned to stable world-space quanta and includes a large hidden prefetch margin beyond the visible-radius slider. It recentres before the old coverage can enter the radar. Raster work is a back buffer: the previous complete grid and contours remain authoritative until a complete replacement is ready for one atomic swap, and ray-fallback progress is diagnostic-only rather than visible as growing chunks. Per-cell height layers reconstruct corridors, ramps and stairs while rejecting unrelated stacked floors; vertical PCB, box, cylinder and plane colliders close walls and generic pull barriers. Contours are stitched across the whole connected grid, simplified to remove raster stair-steps and clipped continuously at the circular radar edge. A bounded scene fingerprint runs between heavier refreshes, while combat transitions and structural map/object signals request immediate replacement; changing floors and barriers are therefore detected even with a stationary actor. The resulting reachability service is shared with safe-position selection and 2D/3D attack rendering: segments known to be outside the reachable surface are clipped and world lines follow reconstructed floor height, while unknown cells never suppress an alert. A separately capped combat radial sweep overlays newly appeared barriers. If native mesh data is absent or streaming, the bounded near-first raycast frontier remains available as a compatibility fallback rather than the normal path.
