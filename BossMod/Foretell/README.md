...
The hybrid guide layer is separate from adaptive learning. `ForetellGuideService` discovers the current instance page from its English ContentFinderCondition name and handles bounded background HTTP/cache work. `ForetellGuideParser` extracts documentary boss/phase/mechanic structure with revision/hash provenance. `ForetellGuides` uses the current content/territory plus live actor OID/BNpcName and Action IDs to bind unique cast entries, then reads official client-language display sheets. Source names without this context remain unbound. Downloading a document does not imply tactical, timing or geometry coverage. No instance-specific catalogue, external launcher, local LLM or GitHub synchronization tool is required or shipped.

Only exact, simple response templates are currently prepared in EN/FR/DE/JA. Complex source prose and all conditions remain available in English and explicitly unprepared. Repeated action names across phases and unresolved variants abstain. Live cast interruption, death and content changes clear applicability. Prepared text is computed on the worker, not by parsing prose in the draw loop. The guide layer does not write learned mechanic priors, route constraints or geometry. See `docs/review-hybrid-guides-2026-09-06.md` at the repository root for validation and the deferred model boundary.
...
The telemetry learner excludes BossModule implementations, state machines, encounter components/layouts/presets and equivalent legacy authored answers. Explicitly attributed external guides are allowed in the separate hybrid layer; source text is not telemetry or evidence of learning success. The in-game coverage audit reports discovered, ingested, learner-used, explicitly excluded and unaccounted fields; unaccounted data is treated as a defect rather than silently ignored.
