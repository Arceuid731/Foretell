    ...
    GuideDocument? Document, GuideCaptureState State, GuideCaptureOptions Options, GuideAdaptedBoss[] Adapted, GuideCapturedSignal[] Signals)
{
    ...
    public MovingHazardState[] MovingHazards { get; init; } = [];
    public int MovingHazardsOmitted { get; init; }
    public long EstimatedBytes => 4096 + (Document?.Bosses.Sum(boss => boss.Phases.Sum(phase => phase.Context.Length + phase.Mechanics.Sum(mechanic => mechanic.Text.Length + mechanic.Name.Length))) ?? 0) * 8L
        ...
        + MovingHazards.Length * 768L + Signals.Sum(signal => 256L + (signal.Boss.Length + signal.Phase.Length + signal.Mechanic.Length + signal.Instruction.Length + signal.Evidence.Length) * 2L)
    ...
    private void WriteGuide(GuideEvent item)
    {
        ...
        if (guideHash != null && guideHash != _guideAttemptedAdaptation)
        {
            ...
            if (TryWriteGuideArtifact(filename, "adapted", new
            {
                ...
                input.IdResolution, input.MovingHazards, input.MovingHazardsOmitted,
        ...
        try
        {
            frame = SerializeGuide(new
            {
                ...
                input.MovingHazards, input.MovingHazardsOmitted,
        ...
        catch (IOException)
        {
            ...
            frame = JsonSerializer.SerializeToUtf8Bytes(new { sequence, input.At, sourceHash = item.SourceHash, guideHash,
                ...
                movingHazardsOmitted = input.MovingHazards.Length + input.MovingHazardsOmitted,
                gap = "Sample exceeds frame size limit; signals, moving hazards, binding audits, options and presentation rows omitted", rejected = item.Session.GuideRejected }, GuideJson);
        }
        ...
        _guideLastInput = input with { Document = null, Adapted = [], Signals = [], Presentation = null, BindingAudits = [], MovingHazards = [],
